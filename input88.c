#include </home/fanisatt/CMAKE/myheader.h>
...
unsigned short int outmen , meg_par, par_leks, big_par, undo_max=5 , fonter=0 , coler=0 , linker = 0 , listx , lister=0 ; 
...
void *mem_undrme, *mem_tent, *mem_arrs , *mem_find ,*mem_fonter , *mem_coler ;
void *mem_efedr1 , *mem_efedr2 , *mem_info , *mem_link , *mem_list ;
int *mem_int ;
char *clipbo ;
char *clipbo1;
int pos[orio+1][2] ;
...
static int gfont1 , alfon ;

if (!alfon) gfont1=gfont ;

if (langex) {gr_flag=1; us_flag=0;}
else {us_flag=1; gr_flag=0;}
...
if (undo==2) {
    ...
    strcat(file_1,"TMP/snapshot.dat");
    fpundo=fopen(file_1,"rb");
    if (fpundo!=NULL) {
        fread (&cur,2,1,fpundo);
        fread (&ar,2,1,fpundo);
...
strcpy(file_1,path_name);
strcat(file_1,"ICD/Colors_17.ico");
mem_coler=icontomem(file_1,255);
strcpy(file_1,path_name);
...
strcpy(file_1,path_name);
strcat(file_1,"ICD/Rlist17.ico");
mem_list=icontomem(file_1,255);
listx=xl+38;
...
_puticon(xr-85,yt+1,mem_coler,255);
_puticon(xl+19,yt+1,mem_link,255);
_puticon(listx,yt+1,mem_list,255);

if (arped==-1) {
    if (keyb_gr) strcpy (str1024,"Σημειωματάριο"); 
    else strcpy (str1024,"Notebook");
    normtext_tomy(str1024,0);
    _outmystr_center_col(0, str1024,xl+37, yt+1, xr-87,yt+19, chrpnt[2],cbrown); 
}
else if (arped==-2) {
    if (keyb_gr) strcpy (str1024,"Δημιουργείστε Λίστα Επιλογής"); 
    else strcpy (str1024,"Create a Selection List");
    normtext_tomy(str1024,0);
    _outmystr_center_col(0, str1024,xl+37, yt+1, xr-87,yt+19, chrpnt[2],cbrown); 
}
else { _outmystr_center_col(0, ptr_pedio[arped]->finame,xl+37, yt+1, xr-87,yt+19, chrpnt[2],cbrown); }
refresh();
yt+=20;
...
if (!mem_enter) { printf ("Out of memory") ; free (mem1); return 0; }
getimage(xep+136,yep+4,xep+179,yep+30,mem_enter);
size=imagesize(xep+182,yep+4,xep+223,yep+30);
mem_tent=malloc(size);
if(!mem_tent) {printf ("Out of memory") ; free (mem1); free(mem_enter); return 0;}
getimage(xep+182,yep+4,xep+223,yep+30,mem_tent);
size=imagesize (xr+3,yt-1,xr+13,yb);
mem_bar=malloc(size);
if (mem_bar==NULL) {printf ("Out of memory") ; free (mem1); free(mem_enter); free(mem_tent); return 0; }
...
// ELEGSOS GIA CAPS LOCK PATHMENO XVRIS MPERDEMA ME ALLA PLHKTRA //
    do {
            SDL_PollEvent(&sdlev);} while ( (sdlev.type==SDL_KEYDOWN) && (SDL_PollEvent(&sdlev)) );

state = SDL_GetKeyboardState(NULL);
ch_mod=SDL_GetModState();

//if (state[SDL_SCANCODE_LSHIFT]) continue;
//if (state[SDL_SCANCODE_RSHIFT]) continue ;
//if (state[SDL_SCANCODE_LALT]) continue;
//if (state[SDL_SCANCODE_RALT]) continue;
//if (state [SDL_SCANCODE_LCTRL]) continue ;
//if (state [SDL_SCANCODE_RCTRL]) continue ;
...
//ELEGXOS TOY ALFARIUMHTIKOY str kai parousiash tou keimenou //
    ...
    else {cur=0;ar=0;vam[0]=0;vam[1]=0;vam[2]=0;wherp=0;pre_cur=0;ins=0;undo=0; undo_firfile=1; undo_lasfile=0 ;}
...
if (!SDL_PollEvent(&sdlev)) continue;
    ...
    if (mx>xl && mx<xl+18 && my>yt-21 && my<yt) { // Στην κεφαλίδα
        ...
        if (inp_info) {puticon(xr-18,yt-20,mem_info,255); inp_info=0;hide_help(hlpxt , hlpyt);}
        else if (fnd_but) {puticon(xr-36,yt-20,mem_find,255); fnd_but=0;hide_help(hlpxt , hlpyt);}
        else if (port) {port=0; putimage(xep+136,yep+4,mem_enter,0);hide_help(hlpxt , hlpyt);}
        else if (tent) {tent=0; putimage(xep+182,yep+4,mem_tent,0);refresh();hide_help(hlpxt , hlpyt);}
        else if (fonter) {fonter=0 ; puticon(xr-54,yt-20,mem_fonter,255); hide_help(hlpxt , hlpyt);}
        else if (coler) {coler=0; puticon(xr-85,yt-20,mem_coler,255); hide_help(hlpxt , hlpyt);}
        else if (linker) {linker=0 ; puticon(xl+19,yt-20,mem_link,255); hide_help(hlpxt , hlpyt);}
        else if (lister) {lister=0 ; puticon(listx,yt-20,mem_list,255); hide_help(hlpxt , hlpyt);}
        ...
        if (!marrs) {
            ...
            if (keyb_gr) show_help("Μετακίνηση Παραθύρου", xl , yt-41 , &hlpxt , &hlpyt) ;
            else show_help("Move Window", xl, yt-41, &hlpxt , &hlpyt) ;
            continue ;}
            else continue ;
        }
    ...
    if (marrs) {
        ...
        hide_help(hlpxt , hlpyt);
 }

    if (mx>xr-18 && mx<xr+1 && my>yt-21 && my<yt) {  // Info Button
        
        if (fnd_but) {puticon(xr-36,yt-20,mem_find,255); fnd_but=0;hide_help(hlpxt , hlpyt);}
        else if (port) {port=0; putimage(xep+136,yep+4,mem_enter,0);hide_help(hlpxt , hlpyt);}
        else if (tent) {tent=0; putimage(xep+182,yep+4,mem_tent,0);refresh();hide_help(hlpxt , hlpyt);}
        else if (fonter) {fonter=0 ; puticon(xr-54,yt-20,mem_fonter,255); hide_help(hlpxt , hlpyt);}
        else if (coler) {coler=0; puticon(xr-85,yt-20,mem_coler,255); hide_help(hlpxt , hlpyt);}
        ...
        if (!inp_info) {
          ...
          if (keyb_gr) show_help("Γενικά στοιχεία για το κείμενο", xr-18 , yt-41 , &hlpxt , &hlpyt) ;
          else show_help("General info for the text", xr-18, yt-41, &hlpxt , &hlpyt) ;
          continue; }
          else continue;
        }
    
    if (inp_info) {
        puticon(xr-18,yt-20,mem_info,255); inp_info=0;hide_help(hlpxt , hlpyt);}
        
            
    if (mx>xr-36 && mx<xr-18 && my>yt-21 && my<yt) {  // Search Button
        
        if (port) {port=0; putimage(xep+136,yep+4,mem_enter,0);refresh(); hide_help(hlpxt , hlpyt);}
        else if (tent) {tent=0; putimage(xep+182,yep+4,mem_tent,0);refresh();hide_help(hlpxt , hlpyt);}
        else if (fonter) {fonter=0 ; puticon(xr-54,yt-20,mem_fonter,255); hide_help(hlpxt , hlpyt);}
        else if (coler) {coler=0; puticon(xr-85,yt-20,mem_coler,255); hide_help(hlpxt , hlpyt);}
        else if (linker) {linker=0 ; puticon(xl+19,yt-20,mem_link,255); hide_help(hlpxt , hlpyt);}
        ...
        if (!fnd_but) {
          ...
          putNOTicon(xr-36,yt-20,mem_find);
            if (arped<0) {
                mytext_tonorm (normdeigma[100] , str1kb , 20) ;
            ...
            {
                mytext_tonorm (normdeigma[arped] , str1kb, 20) ;
                if (keyb_gr) sprintf (str1024 , "Αριστ. κλικ : Αναζήτηση [%s] - Δεξί κλικ : Εισαγωγή του προς αναζήτηση δείγματος",str1kb);
                else sprintf (str1024 , "Left Click : Search [%s] - Right Click : Input text sample for searching" ,str1kb);
            }
        show_help (str1024 , xr-18, yt-41, &hlpxt , &hlpyt) ;
          continue; }
          else continue;
        }
    ...
    if (mx>xr-54 && mx<xr-36 && my>yt-21 && my<yt) {  // Fonts Button
        ...
        if (!fonter) {
          ...
          if (keyb_gr) show_help("Επιλογή Γραμματοσειράς (προσωρινή χρήση)", xr-54 , yt-41 , &hlpxt , &hlpyt) ;
          else show_help("Choose Font (temporary use)", xr-54, yt-41, &hlpxt , &hlpyt) ;
          continue; }
          else continue;
        }
    
    if (fonter) {fonter=0 ; puticon(xr-54,yt-20,mem_fonter,255); hide_help(hlpxt , hlpyt);}
    
    if (mx>xr-85 && mx<xr-54 && my>yt-21 && my<yt) {  // Change Colors Button
        ...
        if (!coler) {
          ...
          SDL_SetCursor(cursor);
          coler=1;
          putNOTicon(xr-85,yt-20,mem_coler);
          if (keyb_gr) show_help("Επιλογή Χρωμάτων Κειμένου-Φόντου-Επιλογής (Προσωρινή Χρήση)", xr-85 , yt-41 , &hlpxt , &hlpyt) ;
          else show_help("Choose Colors (Text - Background - Selection) (temporary use)", xr-85, yt-41, &hlpxt , &hlpyt) ;
          continue; }
          else continue;
        }
    
        if (coler) {coler=0 ; puticon(xr-85,yt-20,mem_coler,255); hide_help(hlpxt , hlpyt);}
        ...
        if (!linker) {
          ...
          if (keyb_gr) show_help("Σύνδεση ΕΠΙΛΕΓΜΕΝΟΥ κειμένου με κατάλληλη εξωτερική εφαρμογή", xl+19 , yt-41 , &hlpxt , &hlpyt) ;
          else show_help("Link of SELECTED text with an appropriate external application", xl+19 , yt-41 , &hlpxt , &hlpyt) ;
          continue; }
          else continue;
        }
    
    if (linker) {linker=0 ; puticon(xl+19,yt-20,mem_link,255); hide_help(hlpxt , hlpyt);}
    
    if (mx>listx && mx<listx+18 && my>yt-21 && my<yt) {  // LISTER !! (Έτοιμες Λίστες φορτωμένες)
        ...
        if (!lister) {
            ...
            SDL_SetCursor(cursor);
            lister=1;
            putNOTicon(listx,yt-20,mem_list);
            if (keyb_gr) show_help("Άνοιγμα Φορτωμένης Λίστας - Σύνδεση μέσω clipboard", listx , yt-41 , &hlpxt , &hlpyt) ;
            else show_help("Open Loaded List - Link via Clipboard", listx , yt-41 , &hlpxt , &hlpyt) ;
            continue; }
            else continue;
    }
    
    if (lister) {lister=0 ; puticon(listx,yt-20,mem_list,255); hide_help(hlpxt , hlpyt);}
        
    if (mx>xep+135 && mx<xep+180 && my>yep && my<yet) { // Κλείσε παράθυρο εισαγωγής
        
        if (tent) {tent=0; putimage(xep+182,yep+4,mem_tent,0);refresh();hide_help(hlpxt , hlpyt);}
        ...
        _putpixel (xep+i,yep+f);}} 
        refresh(); 
        if (keyb_gr) show_help("Kλείσιμο παραθύρου", xep+135 , yet+1 , &hlpxt , &hlpyt) ;
        else show_help("Close window", xep+135 , yet+1, &hlpxt , &hlpyt) ;
        continue ;}
        else continue ;
        continue; }
    ...
    if (mx>xep+182 && mx<xet && my>yep && my<yet) { // Αλλαγή Διαστάσεων
        ...
        _putpixel (xep+i,yep+f);}} 
        refresh(); 
        if (keyb_gr) show_help("Αλλαγή Διαστάσεων", xep+182 , yet+1 , &hlpxt , &hlpyt) ;
        else show_help("Change Dimensions",  xep+182 , yet+1 , &hlpxt , &hlpyt) ;
        continue ;}
        else continue ;
        continue; }

    if (tent) {tent=0; putimage(xep+182,yep+4,mem_tent,0);hide_help(hlpxt , hlpyt);}
    
    if ( (keyb_gr) && mx>xep+2 && mx<xep+43 && my>yep && my<yet) {
        if (mem_small) hide_help(hlpxt , hlpyt);
        cursor = SDL_CreateSystemCursor(SDL_SYSTEM_CURSOR_HAND);
  ...
  if (vam[0]) {                              // βάφω 
    ...
    if(sdlev.button.button==SDL_BUTTON_LEFT) {   
        ...
        if (mx>xep && mx<xet && my>yb && my<yet) { // Choose ok ,flag , change dimensions
            ...
            switch (diaf1) {
                ...
                case 3: // (FYGE (Return) me pontiki)
                    ...
                    putimage(xl-1,yt-21,mem1,0); putimage(xep,yep,mem_undscr,0);free(mem_undscr);free(mem1) ; free(mem_cu); free(mem_flusa); free(mem_enter);free(mem_tent);  free(mem_under); if (bar) putimage (xr+3,yt-1,mem_bar,0);free(mem_bar); if (keyb_gr) free(mem_flgr); free(mem_rmenu); free(mem_arrs); free(mem_info); str[ar]=255;
                    free(mem_find); free(mem_fonter); free(mem_coler);free(mem_link); free(mem_list) ;
                    langex = (us_flag) ? 1 : 2 ;
                    cursor = SDL_CreateSystemCursor(SDL_SYSTEM_CURSOR_ARROW);    SDL_SetCursor(cursor);refresh(); diaf1=ar; ar=0; undo=0; undo_firfile=1; undo_lasfile=0 ; cur=0;vam[0]=0;vam[1]=0;vam[2]=0;wherp=0;pre_cur=0;ins=0; alfon=0; return diaf1;
        ...
        if (coler) {
                    ...
                    if(diak) {putimage(x,y,mem_cu,0); refresh(); diak=0;}
                    for (f=0 ; f<3 ; f++) { proxchxr[f]=chxr[f] ; proxbkxr[f]=bkxr[f] ; proxslxr[f]=slxr[f] ; }  
                    inkualizer (gfont1, proxchxr , proxbkxr , proxslxr) ;
                    i=0;
                    for (f=0 ; f<3 ; f++) {
                        if (proxchxr[f] != chxr[f]) { i=1; break; }
                        if (proxbkxr[f] != bkxr[f]) { i=1; break; }
                        if (proxslxr[f] != slxr[f]) { i=1; break; }
                    }
                    if (!i) continue;
                    for (f=0 ; f<3 ; f++) { chxr[f] = proxchxr[f] ; bkxr[f] = proxbkxr[f] ; slxr[f] = proxslxr[f]; }
                    free(mem_cu); 
                    ...
                    free(mem1);
                    refresh();
                    return -10 ; // EIDIKH PERIPTVSH EPISTROFHS ALLAGH XRVMATVN !!!
        }
        ...
        if (linker) {
            ...
            user_ans=smmenu_show_opt(11, submenu, opt_men, str1024, xl+37,yt-20);
            for (f=0 ; f<3 ; f++) { chxr[f] = proxchxr[f] ; bkxr[f] = proxbkxr[f] ; slxr[f] = proxslxr[f]; }
            if (user_ans<0) continue ;
            size=0;
            ...
            size+=2;
            clipbo=malloc(size);
            clipbo1=malloc(size+1);
            if ( (!clipbo) || (!clipbo1) ) {information(2-keyb_gr,NULL,NULL); continue ; }
            i=0;
            for (f=vam[1] ; f<=vam[2] ; f++) {clipbo[i++]=str[f];}
            clipbo[i]=255;
            mytext_tonorm(clipbo,clipbo1,size);
            i=0;f=vam[1];
            what=str[f];
            do {
                if (what>163) break ;
                str1024[i++]=what;
                f++; what=str[f];
            }while (f<=vam[2] && i<1023);
            str1024[i]=255;
            mytext_tonorm(str1024,str1kb,1024);
            i=0;
            switch (user_ans) { 

                case 0:
                            i=SDL_SetClipboardText(clipbo1);
                            i = (!i) ? 1: 0 ;                       
                    if (i) {
                ...
                case 1:
                    ...
                    if (pid<0) {perror("Fork...");break;}
                        ...
                        execl(orisma[0],orisma[1],orisma[2],(char *)NULL); 
                        _exit (1); 
                        }
               ...
               case 6:
                ...
                case 7: 
                    ...
                    if (pid<0) {perror("Fork...");break;}
                        ...
                        execl("/usr/bin/vlc","vlc","--play-and-exit",orisma[2],(char *)NULL); 
                        _exit (1); 
                        }
                    else waitpid(pid,&status,WNOHANG);
                ...
                case 8 :
                    ...
                    if (pid<0) {perror("Fork...");break;}
                        ...
                        //pid=getpid();
                        execl("/usr/bin/cvlc","cvlc",orisma[2],(char *)NULL); 
                        _exit (1); 
                        }
                    else waitpid(pid,&status,WNOHANG);
                    break ;
            ...
            free(clipbo) ; free(clipbo1) ;
            continue ;            
                
        }
        
        if (lister) {
            if (dbmaxbut<0) continue ;
            lister=0;
            if (mem_small) hide_help(hlpxt , hlpyt);
            if(diak) {putimage(x,y,mem_cu,0); refresh(); diak=0;}
            for (f=0 ; f<3 ; f++) { proxchxr[f]=chxr[f] ; proxbkxr[f]=bkxr[f] ; proxslxr[f]=slxr[f] ; }
            ...
            i=open_buts.butt_elem;
            if(i>=0) {                        // (kati<0) no button_show
            what=open_buts.butt_data[0];
            if (what !=255) {                       // Επιλογή κάποιας λίστας (i) για παρουσίαση !
                dbbut[i].butt_elem = button_show ( dbbut[i].mem_butt , dbbut[i].butt_data , dbbut[i].butt_ar , dbbut[i].butt_orio , dbbut[i].butt_name , &dbbut[i].butt_sorton , 0 );}
            }
             for (f=0 ; f<3 ; f++) { chxr[f] = proxchxr[f] ; bkxr[f] = proxbkxr[f] ; slxr[f] = proxslxr[f]; }
            puticon(listx,yt-20,mem_list,255);
            continue ;
        }
        ...
        SDL_SetCursor(cursor); continue;}
            ...
            switch (kyk) {
                ...
                case 0: 
                    ...
                    if (savefile[0]=='\0') sprintf (file_2,"/home/%s/",user_name);
                    else {
                        i=strlen(savefile);
                    ...
                    f = fileselector( big_char, savefile , 1024);
                ...
                case 1:
                    ...
                    fclose(fpundo);
                        ...
                        strcpy(file_2,path_name);
                        strcat(file_2,"TMP/snapshot.dat");
                        fpundo=fopen(file_2,"rb");
                        if (fpundo!=NULL) {fclose (fpundo); remove (file_2);}
                        
                        a=undo_lasfile-undo_max;
                        sprintf(file_1,"%s%s%d%s",path_name,"TMP/undo",a,".dat");
                ...
                case 6: // Copy to Clipboard
                    ...
                    for (f=vam[1] ; f<=vam[2] ; f++) {
                        ...
                        size++;
                        clipbo=malloc(size);
                        if (!clipbo) {information(2-keyb_gr,0,0) ; continue;}
                        a=0;
                        for (f=vam[1] ; f<=vam[2] ; f++) {
                        what=str[f];
                        if (what<95) {
...
DROMO:
                ...
                free(mem_under); free(mem_flusa);  free(mem_enter); free(mem_tent);if (keyb_gr) free(mem_flgr); if (bar) putimage (xr+3,yt-1,mem_bar,0);free(mem_bar);free(mem_rmenu);
                free(mem_find); free(mem_fonter); free(mem_coler); free(mem_link);free(mem_list) ;
                langex = (us_flag) ? 1 : 2 ;
                ...
                cur=0;vam[0]=0;vam[1]=0;vam[2]=0;wherp=0;pre_cur=0;ins=0; alfon=0; if (tab) return -9; else return 0;
        ...
        break;
                    ...
                    case 12416 : 
                                ...
                                for (f=vam[1] ; f<=vam[2] ; f++) {
                                    ...
                                    size++;
                                    clipbo=malloc(size);
                                    if (!clipbo) { information(2-keyb_gr,0,0) ; break; }
                                    a=0;
                                    for (f=vam[1] ; f<=vam[2] ; f++) {
                                    what=str[f];
                                    if (what<95) {
                    ...
                    free(mem_bar); 
                    if (keyb_gr) free(mem_flgr); 
                    free(mem_rmenu); free(mem_arrs); free(mem_info);free(mem_find); free(mem_fonter); free(mem_coler); free(mem_link);free(mem_list) ;
                    str[ar]=255;
                    if (inp_mem_ghost) { free(inp_mem_ghost); inp_mem_ghost=0; }
                    langex = (us_flag) ? 1 : 2 ;
                    cursor = SDL_CreateSystemCursor(SDL_SYSTEM_CURSOR_ARROW);    SDL_SetCursor(cursor);refresh(); diaf1=ar; ar=0; undo=0; undo_firfile=1; undo_lasfile=0 ; cur=0;vam[0]=0;vam[1]=0;vam[2]=0;wherp=0;pre_cur=0;ins=0; alfon=0; return diaf1;
...
size_cu=imagesize(0,0,1,(bh-1));
mem_cu=malloc(size_cu);
if (!mem_cu) { information(2-keyb_gr,0,0) ; free(mem1); return 0; }
getimage(xl,yt,xr,yb,mem1);
x=xl ; y=yt;
//diaf1=yb-yt+1; diaf2=diaf1/bh;maxline=diaf2; ektos=yt+(diaf2*bh);
...
size=imagesize (xr+3,yt-1,xr+13,yb);
mem_bar=malloc(size);
if (mem_bar==NULL)  { printf ("Out of memory") ; free (mem1); free(mem_undscr); free(mem_enter); free(mem_tent); return 0; }
getimage (xr+3,yt-1,xr+13,yb,mem_bar);
        ...
        if (mx>xep && mx<xet && my>yb && my<yet) { // Choose ok ,flag , change dimensions
            ...
            switch (diaf1) {
                ...
                case 3: // (FYGE (Return) me pontiki)
                    ...
                    free(mem_find); free(mem_fonter); free(mem_coler);free(mem_link); free(mem_list) ;
                    langex = (us_flag) ? 1 : 2 ;
                    cursor = SDL_CreateSystemCursor(SDL_SYSTEM_CURSOR_ARROW);    SDL_SetCursor(cursor);refresh(); ar=0; undo=0; undo_firfile=1; undo_lasfile=0 ; cur=0;vam[0]=0;vam[1]=0;vam[2]=0;wherp=0;pre_cur=0;ins=0; alfon=0; return diaf1;
...
DROMO:
                ...
                free(mem_arrs); free(mem_info); cursor = SDL_CreateSystemCursor(SDL_SYSTEM_CURSOR_ARROW);
                SDL_SetCursor(cursor);refresh() ; ar=0;undo=0; undo_firfile=1; undo_lasfile=0 ;
                cur=0;vam[0]=0;vam[1]=0;vam[2]=0;wherp=0;pre_cur=0;ins=0; alfon=0; if (tab) return -9; else return diaf1 ;
...
areacls(xl,yt,xr,yb,bkr,bkg,bkb);
if (!ptr_pedio[arped]->fitype) _bareacls (xr+2,yt-1,xr+12,yb,dbbkxr);
x=xl ; y=yt;
// diaf1=yb-yt+1; diaf2=diaf1/bh;maxline=diaf2; ektos=yt+(diaf2*bh);
maxline=1;ektos=yt+bh;
...
if (arped==-1) {
    if (keyb_gr) strcpy (str1024,"Αριθμητική Παράσταση"); 
    else strcpy (str1024,"Numeric Equation");
    normtext_tomy(str1024,0);
    _outmystr_center_col(0, str1024,xl+37, yt+1, xr-87,yt+19, chrpnt[2],cbrown); 
}
